...
export default function SaleStatus() {
  ...
  const [isWorking, setIsWorking] = useState<"idle" | "saving" | "sharing">(
    "idle"
  );
  ...
  const FILENAME = `rates-status-${format(
    getIndianTime(),
    "yyyyMMdd-HHmm"
  )}.png`;
  ...
  // ✅ FIXED Save: works on mobile (no stuck screen)
  ...
  const handleShareWhatsApp = async () => {
    try {
      ...
      // Native share (Chrome/Edge mobile, Safari iOS 16.4+)
      // @ts-expect-error
      if (navigator?.share) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="shrink-0 bg-gradient-to-r from-gold-600 to-gold-700 text-black px-3 py-2 md:px-4 md:py-3 flex justify-center items-center shadow-md">
        <img
          src="/logo.png"
